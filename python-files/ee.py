...
def add_dummy_data(input_file, target_size_mb):
    ...
    try:
        ...
        target_size_bytes = target_size_mb * 1024 * 1024  # Конвертируем МБ в байты
        ...
        with open(output_file, 'ab') as f:
            ...
            while bytes_to_add > 0:
                chunk = min(chunk_size, bytes_to_add)
        ...
        input("Нажмите Enter, чтобы закрыть...")  # Для .exe, чтобы окно не закрылось сразу

def main():
    print("Программа для увеличения размера файлов")
    print("Поддерживаемые форматы: .jpg, .jpeg, .png, .mp4, .avi, .txt")
    
    # Запрашиваем у пользователя путь к файлу
    input_file = input("Введите путь к файлу: ").strip()
    
    # Проверяем, является ли файл допустимым
    valid_extensions = {'.jpg', '.jpeg', '.png', '.mp4', '.avi', '.txt'}
